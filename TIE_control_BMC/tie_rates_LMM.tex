...
Since the Wald test statistic $t^*$ is scaled by the standard error of $\hat{\beta}_1$, and since that standard error is proportional to the square root of the total outcome variance through the ICC, different magnitudes of $\sigma_b^2$ and $\sigma^2$ that gave the same ICC will produce the same test statistics. A test of different magnitudes for $\sigma_b^2$ and $\sigma^2$ that produced the same ICC confirmed this, allowing us to simplify our analysis by fixing $\sigma^2$ at 1 and only varying $\sigma^2_b$.
